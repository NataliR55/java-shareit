...
public interface BookingService {
    ...
    List<OutputBookingDto> getBookingsOfBooker(State state, Long bookerId, int from, int size);

    List<OutputBookingDto> getBookingsOfOwner(State state, Long ownerId, int from, int size);
}
